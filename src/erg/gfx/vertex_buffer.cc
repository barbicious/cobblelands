...
namespace erg::gfx {
    VertexBuffer::VertexBuffer(const std::span<f32> vertices, const std::span<i32> vertex_attrib_sizes) : Buffer{GL_ARRAY_BUFFER} {
        ...
        for (const i32& vertex_attrib_size : vertex_attrib_sizes) {
            glVertexAttribPointer(i, vertex_attrib_size, GL_FLOAT, GL_FALSE, stride * sizeof(f32), reinterpret_cast<void*>(last_size * sizeof(f32)));
